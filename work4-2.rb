...
get '/work/4-2/?page=:id' do
  ...
  #ページ数
  @pagecountall = @countall / 5

  #【"前へ" ページが０の時非表示にする】◯以上のとき以外は非表示
  unless params[:id].to_i <= 0
   @prev = "<a href='/work/4-2/page=#{params[:id].to_i - 1 }'><<前へ</a>"
  end

  #【"次へ" ページが最後の時非表示にする】◯以上のときは非表示
  unless params[:id].to_i <= 0 || params[:id].to_i >= @pagecountall
  @next = "<a href='/work/4-2/page=#{params[:id].to_i + 1 }'>>>次へ</a>"
  end
